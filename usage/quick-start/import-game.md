...
同一款游戏可以同时存在于多个本地游戏库，也可以同时保留本地文件夹版和 Steam 版。
PotatoVN 会将它们显示为同一个游戏下的多个安装实例，游戏信息、游玩状态和总游玩时间会共享，
但运行文件、启动参数、进程、转区/DPI 以及存档路径可以分别设置。

- 信息源 ID 明确匹配时，新路径会自动关联为已有游戏的另一个安装。
- 只有名称相同时，扫描结果会要求您确认后再关联，避免误合并。
- 游戏详情页的开始按钮会启动上次成功使用的安装；通过按钮右侧菜单可选择其他安装。
- 在游戏设置的“安装实例”区域可以添加、设为首选、打开或从库中移出某个安装。
- 从库中移出最后一个安装不会删除游戏信息；游戏会保留为非本地游戏。删除磁盘文件和永久移除游戏是独立操作。
